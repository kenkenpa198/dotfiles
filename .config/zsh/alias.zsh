...
alias gbc="git fetch --prune && git branch --merged | grep --invert-match '*' | xargs --no-run-if-empty git branch -d" # マージ済みのブランチを削除 cf. https://e-penguiner.com/remove-local-branches-not-on-remote/
