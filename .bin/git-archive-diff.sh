...
# - v2.0.0
#     - シェルスクリプトとして刷新。
#     - 結果の出力へアーカイブしたファイルの一覧表示を追加。
#     - 変更後のコミットを指定しなかった場合は HEAD との差分を出力するように更新。
#     - アーカイブのファイル名指定処理を削除。
...
function do_git_archive() {
    ...
    # ファイル名にスペースが含まれていた場合に別の要素として変数へ代入される。
    # この場合 git archive コマンド実行時の変数展開の際に別のファイル名として扱われてしまい実行に失敗する。
    # ----------------------------------------------------------------------------------------------
